...
export default defineSchema({
  ...
  // ===============================
  // SMART CONVERSION AUTOMATION (BULLETPROOF)
  // ===============================

  smartConversionAutomations: defineTable({
    name: v.string(),
    clientIdentifier: v.string(), // Domain or client ID
    scheduledTime: v.string(), // "HH:MM" format (24-hour)
    ...
    lastExecutionStatus: v.optional(v.string()), // "success", "failed", "no_leads", "error"
    lastExecutionResult: v.optional(v.object({
      success: v.boolean(),
      leadsFound: v.number(),
      leadsConverted: v.number(),
      error: v.optional(v.string()),
    ...
    // Timestamps
    createdAt: v.number()
  }).index("by_client", ["clientIdentifier"])
    .index("by_scheduled_time", ["scheduledTime"])
    .index("by_active", ["isActive"])
    .index("by_client_active", ["clientIdentifier", "isActive"]),

  smartConversionExecutions: defineTable({
    automationId: v.id("smartConversionAutomations"),
    clientIdentifier: v.string(),
    
    // Execution details
    executedAt: v.number(),
    executionDuration: v.number(), // milliseconds
